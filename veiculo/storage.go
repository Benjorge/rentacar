...
//interface é um contrato de implementação
type Storage interface {
	GetVeiculos() ([]Veiculo, error)
	CreateVeiculos(nome, marca string, ano, modelo int) error
...
// func (dono) nomeFuncao (parametros) (retorno)
...
func (s *MySQLStorage) UpdateVeiculo(id int, veiculo *Veiculo) error {
	...
	stmt, err := s.dbConn.Prepare(update) // prepare: prepara o banco de dados para receber os parametros
	if err != nil {
		return err
	}
	defer stmt.Close()
